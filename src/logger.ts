...
function mergePkgOptimizeTags(pkgTags: string[]): void {
  ...
  const withoutPkg = parts.filter((p) => p !== NS && !p.startsWith(`${NS}:`));
...
export function primeErrorDebug(): void {
  const parts = splitDebugList();
  if (parts.some((p) => p === `${NS}:error` || p === NS || p === `${NS}:*`)) {
    debug.enable(process.env.DEBUG ?? "");
    return;
  }
...
export function configureLogging(opts: {
  ...
  const parts = splitDebugList();
  const withoutPkg = parts.filter((p) => p !== NS && !p.startsWith(`${NS}:`));
  const existingPkg = parts.filter((p) => p === NS || p.startsWith(`${NS}:`));
  ...
  const defaults = [`${NS}:error`, `${NS}:warn`, `${NS}:info`, `${NS}:result`];
  const hasWildcard = existingPkg.some((p) => p === `${NS}:*` || p === NS);
