...
const PRESETS: Record<string, Partial<PackageConfig>> = {
  ...
  'pkg-optimize/presets/graphql-request': graphqlRequest as Partial<PackageConfig>,
  'pkg-optimize/presets/graphql-codegen': graphqlCodegen as Partial<PackageConfig>,
...
const PACKAGE_PATTERNS: Array<{ pattern: RegExp; preset: string }> = [
  ...
  { pattern: /^@tanstack\/react-query$/, preset: 'pkg-optimize/presets/react-query' },
  { pattern: /^@tanstack\/query-/, preset: 'pkg-optimize/presets/react-query' },
  { pattern: /^swr$/, preset: 'pkg-optimize/presets/swr' },
  { pattern: /^graphql-request$/, preset: 'pkg-optimize/presets/graphql-request' },
  { pattern: /^@graphql-codegen\//, preset: 'pkg-optimize/presets/graphql-codegen' },
...
export function matchPreset(targetPackage: string): Partial<PackageConfig> | null {
  const match = PACKAGE_PATTERNS.find((p) => p.pattern.test(targetPackage));
  return match ? loadPreset(match.preset) : null;
}

export function listPresetNames(): string[] {
  return Object.keys(PRESETS).filter((k) => k.startsWith('pkg-optimize/presets/'));
}
