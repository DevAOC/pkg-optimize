...
export async function resolvePackageConfig(
  ...
  const explicitPreset = pkgConfig.extends ? loadPreset(pkgConfig.extends) : null;
  const autoPreset = explicitPreset ?? matchPreset(pkgConfig.targetPackage);
  const detected = await detectPackageConfig(pkgConfig.targetPackage, projectRoot);
...
export function deepMerge<T extends object>(...sources: Array<Partial<T> | object>): T {
